...
class skeleton:
    ...
    def process_each_image(self, heatmap, paf, images):
        ...
        Results = []
        for imag in range(len(images)):
            oriImg = images[imag]
            paf_avg = paf[imag]
            heatmap_avg = heatmap[imag]
                ...
                if (nA != 0 and nB != 0):
                    ...
                    for i in range(nA):
                        for j in range(nB):
                            ...
                            criterion1 = len(np.nonzero(score_midpts > self.param['thre2'])[0]) > 0.8 * len(
                                score_midpts)
            ...
            # VISUALIZACION
            canvas = images[imag]  # B,G,R order
            K = {}
            l = []
            for i in range(18):
		...
		n=0	
            ...
            for i in range(17):
                ...
                canvas = cv.addWeighted(canvas, 0.4, cur_canvas, 0.6, 0)
            Results.append((K, canvas))
        return Results

    def get_skeleton_multiple(self, images,scales):
        oriImg = images[0]
        multiplier = [x * self.model['boxsize'] / oriImg.shape[0] for x in scales]
        heatmap_avg = [np.zeros((oriImg.shape[0], oriImg.shape[1], 19))] * len(images)
        paf_avg = [np.zeros((oriImg.shape[0], oriImg.shape[1], 38))] * len(images)
        for m in range(len(multiplier)):
            ...
            for i in range(len(images)):
                ...
                imageToTest_padded, pad = util.padRightDownCorner(imageToTest, self.model['stride'],
                                                                  self.model['padValue'])
                im = np.transpose(np.float32(imageToTest_padded[:, :, :]), ( 2, 0, 1)) / 256 - 0.5
                in_im.append(im)
            self.net.blobs['data'].reshape(*(len(images), 3, imageToTest_padded.shape[0], imageToTest_padded.shape[1]))
            self.net.blobs['data'].data[...] = in_im
            output_blobs = self.net.forward()
            for i in range(len(images)):
                oriImg = images[i]
                heatmap = np.transpose(np.squeeze(self.net.blobs[output_blobs.keys()[1]].data)[i, :, :, :],
                ...
                paf = np.transpose(np.squeeze(self.net.blobs[output_blobs.keys()[0]].data)[i, :, :, :],
                                   (1, 2, 0))  # output 0 is PAFs
                paf = cv.resize(paf, (0, 0), fx=self.model['stride'], fy=self.model['stride'],
                                interpolation=cv.INTER_CUBIC)
                paf = paf[:imageToTest_padded.shape[0] - pad[2], :imageToTest_padded.shape[1] - pad[3], :]
                paf = cv.resize(paf, (oriImg.shape[1], oriImg.shape[0]), interpolation=cv.INTER_CUBIC)

                heatmap_avg[i] = heatmap_avg[i] + heatmap / len(multiplier)
                paf_avg[i] = paf_avg[i] + paf / len(multiplier)
        return self.process_each_image(heatmap_avg, paf_avg, images)
